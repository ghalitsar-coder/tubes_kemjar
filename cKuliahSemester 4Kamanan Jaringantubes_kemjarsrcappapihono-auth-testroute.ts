...
// Create a Hono app for this route with the correct type
const app = new Hono<{
  Variables: {
    clerkUserId?: string;
    sessionToken?: string;
...
app.get("/", async (c) => {
  ...
  if (clerkUserId) {
    try {
      ...
      const users = await prisma.$queryRaw\`SELECT id, name, email, "isOAuthUser" FROM "User" WHERE "clerkId" = \${clerkUserId}\`;
